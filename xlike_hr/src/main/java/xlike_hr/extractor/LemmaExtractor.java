...
import xlike_hr.extractor.tools.LemmaTagger;
//import xlike_hr.extractor.tools.PosTagger;
import xlike_hr.model.Conll;
import xlike_hr.model.ConllSentence;

public class LemmaExtractor {
	private static Logger log = LogManager.getLogger(LemmaExtractor.class);
...
//	}
	...
	//TIN
	public void extractPosTags(Conll conll) 
	{
		log.debug("Processing pos...");
		for (int sentenceIndex : conll.getSentences().keySet()) {
			ConllSentence conllSentence = conll.getSentences().get(
					sentenceIndex);
			String[] tokens = conllSentence.getWordList();
			lemmaTagger.getPos(conllSentence, tokens);
		}
	...
	private void extractPosTags(Conll conll)
	{
		for(int sentenceIndex: conll.getSentences().keySet())
		{
			...
			for (TaggedWord t : tSentence) 
			{
				...
				conllSentence.updateTokenPosTag(tokenIndex, t.word(), t.tag(), t.beginPosition(), t.endPosition());
				tokenIndex++;
			}
		
			log.debug("POS tagger output conll:\n" + conll.toString());
		}
	}*/
}
